...
import * as ReActor_Tracer$Reactor from "./ReActor_Tracer.bs.js";
...
function nextPid(param) {
  ...
  return ReActor_Process$Reactor.Pid[/* make */0](match[0], match[1], param[/* process_count */2] + 1 | 0);
}
...
function spawn(f, args, scheduler) {
  ...
  var $$process = ReActor_Process$Reactor.make(pid, f, args);
...
function exit(pid, scheduler) {
  return ReActor_Process$Reactor.markAsDead(List.find((function (p) {
...
function send(pid, msg, scheduler) {
  ...
  if (match !== undefined) {
    ReActor_Tracer$Reactor.trace(match, pid, msg);
  }
  return ReActor_Process$Reactor.send(msg, List.find((function (p) {
                    return Caml_obj.caml_equal(p[/* pid */0], pid);
                  }), scheduler[0][/* processes */1]));
}
...
/* FFI_Runtime-Reactor Not a pure module */
