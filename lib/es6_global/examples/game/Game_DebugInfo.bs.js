// Generated by BUCKLESCRIPT VERSION 4.0.5, PLEASE EDIT WITH CARE

import * as Block from "../../../../node_modules/bs-platform/lib/es6/block.js";
import * as Curry from "../../../../node_modules/bs-platform/lib/es6/curry.js";
import * as Option from "./Option.bs.js";
import * as ReActor from "../../src/ReActor.bs.js";
import * as Game_Renderer from "./Game_Renderer.bs.js";
import * as Caml_exceptions from "../../../../node_modules/bs-platform/lib/es6/caml_exceptions.js";

var debugInfo = "debug_info";

var Status = Caml_exceptions.create("Game_DebugInfo.Status");

function handleMessage(state, param) {
  if (param[0] === Status) {
    var data = param[1];
    if (typeof data === "number") {
      return state;
    } else {
      switch (data.tag | 0) {
        case 0 : 
            return /* record */[
                    /* lastKeyName */data[0],
                    /* lastKeyCode */data[1],
                    /* lastMouseClick */state[/* lastMouseClick */2],
                    /* lastMouseMove */state[/* lastMouseMove */3],
                    /* lastScreenSize */state[/* lastScreenSize */4]
                  ];
        case 1 : 
            return /* record */[
                    /* lastKeyName */state[/* lastKeyName */0],
                    /* lastKeyCode */state[/* lastKeyCode */1],
                    /* lastMouseClick *//* tuple */[
                      data[0],
                      data[1]
                    ],
                    /* lastMouseMove */state[/* lastMouseMove */3],
                    /* lastScreenSize */state[/* lastScreenSize */4]
                  ];
        case 2 : 
            return /* record */[
                    /* lastKeyName */state[/* lastKeyName */0],
                    /* lastKeyCode */state[/* lastKeyCode */1],
                    /* lastMouseClick */state[/* lastMouseClick */2],
                    /* lastMouseMove *//* tuple */[
                      data[0],
                      data[1]
                    ],
                    /* lastScreenSize */state[/* lastScreenSize */4]
                  ];
        case 3 : 
            return /* record */[
                    /* lastKeyName */state[/* lastKeyName */0],
                    /* lastKeyCode */state[/* lastKeyCode */1],
                    /* lastMouseClick */state[/* lastMouseClick */2],
                    /* lastMouseMove */state[/* lastMouseMove */3],
                    /* lastScreenSize *//* tuple */[
                      data[0],
                      data[1]
                    ]
                  ];
        
      }
    }
  } else {
    return state;
  }
}

function render(state) {
  var color = /* RGBA */Block.__(2, [
      255,
      255,
      255,
      0.8
    ]);
  var ops_000 = /* Fill */Block.__(0, [/* Hex */Block.__(0, [0])]);
  var ops_001 = /* :: */[
    /* DrawText */Block.__(1, [
        "Key Name: " + (String(state[/* lastKeyName */0]) + ""),
        /* Point2D */[
          10,
          10
        ],
        color
      ]),
    /* :: */[
      /* DrawText */Block.__(1, [
          "Key Code: " + (String(state[/* lastKeyCode */1]) + ""),
          /* Point2D */[
            10,
            20
          ],
          color
        ]),
      /* :: */[
        /* DrawText */Block.__(1, [
            "Click: " + (String(state[/* lastMouseClick */2]) + ""),
            /* Point2D */[
              10,
              30
            ],
            color
          ]),
        /* :: */[
          /* DrawText */Block.__(1, [
              "Move: " + (String(state[/* lastMouseMove */3]) + ""),
              /* Point2D */[
                10,
                40
              ],
              color
            ]),
          /* :: */[
            /* DrawText */Block.__(1, [
                "Screen: " + (String(state[/* lastScreenSize */4]) + ""),
                /* Point2D */[
                  10,
                  50
                ],
                color
              ]),
            /* [] */0
          ]
        ]
      ]
    ]
  ];
  var ops = /* :: */[
    ops_000,
    ops_001
  ];
  Option.$great$great$pipe(ReActor.where_is(Game_Renderer.name), (function (pid) {
          return ReActor.send(pid, [
                      Game_Renderer.Pipeline,
                      ops
                    ]);
        }));
  return /* () */0;
}

function loop(env, state) {
  var state$prime = Option.$less$pipe$great(Option.$great$great$pipe(Curry._1(env[/* recv */1], /* () */0), (function (param) {
              return handleMessage(state, param);
            })), state);
  render(state$prime);
  return /* Suspend */Block.__(2, [
            16,
            state$prime
          ]);
}

function report(e) {
  Option.$great$great$pipe(ReActor.where_is(debugInfo), (function (pid) {
          return ReActor.send(pid, [
                      Status,
                      e
                    ]);
        }));
  return /* () */0;
}

function start() {
  return ReActor.register(debugInfo, ReActor.spawn(loop, /* record */[
                  /* lastKeyName */undefined,
                  /* lastKeyCode */undefined,
                  /* lastMouseClick */undefined,
                  /* lastMouseMove */undefined,
                  /* lastScreenSize */undefined
                ]));
}

export {
  debugInfo ,
  Status ,
  handleMessage ,
  render ,
  loop ,
  report ,
  start ,
  
}
/* ReActor Not a pure module */
